...
class Edrone():
	...
	def __init__(self):
		...
		self.change_error=[0,0,0,0]
		self.Pid_value=[0,0,0,0]
		...
		self.command_pub = rospy.Publisher('/alt_error', Float64, queue_size=1)
		self.command_pub = rospy.Publisher('/pitch_error', Float64, queue_size=1)
		self.command_pub = rospy.Publisher('/roll_error', Float64, queue_size=1)
		self.command_pub = rospy.Publisher('/yaw_error', Float64, queue_size=1)
	...
	#	8. Add error_sum
                ...
                '''sum_error[0]=sum_error+error[0]'''

                self.change_error[1]=self.error[1]-self.prev_error[1]
                '''sum_error[1]=sum_error[1]+error[1]'''

                self.change_error[2]=self.error[2]-self.prev_error[2]

                self.change_error[3]=self.error[3]-self.prev_error[3]

                


                self.Pid_value[0]= self.Kp[0]*self.error[0]+self.Ki[0]*self.sum_error[0]+self.Kd[0]*self.change_error[0]
 
               self.out_pitch=self.Pid_value[0]

                self.Pid_value[1]= self.Kp[1]*self.error[1]+self.Ki[1]*self.sum_error[1]+self.Kd[1]*self.change_error[1]
                self.out_roll=self.Pid_value[1]


                self.Pid_value[2]= self.Kp[2]*self.error[2]+self.Ki[2]*self.sum_error[2]+self.Kd[2]*self.change_error[2]
                self.out_Throttle=self.Pid_value[2]

                self.Pid_value[3]= self.Kp[3]*self.error[3]+self.Ki[3]*self.sum_error[3]+self.Kd[3]*self.change_error[3]
                self.out_Yaw=self.Pid_value[3]
                ...
                elif self.cmd.rcThrottle<1200:
                        self.cmd.rcThrottle=1200
            


            self.cmd.rcYaw= 1500 + self.out_Yaw
                if self.cmd.rcYaw>1800:
                        self.cmd.rcYaw=1800
                        

                elif self.cmd.rcYaw<1200:
                        self.cmd.rcYaw=1200
